...
export default class EventIndex extends EventEmitter {
    ...
    public async addInitialCheckpoints(): Promise<void> {
        ...
        const encryptedRooms = rooms.filter((room: Room) => client.isRoomEncrypted(room.roomId));
     ...
     *         live events
     */
    private onSync = async (state: SyncState, prevState: SyncState): Promise<void> => {
        const indexManager = PlatformPeg.get().getEventIndexingManager();

        if (prevState === SyncState.Prepared && state === SyncState.Syncing) {
        ...
        if (prevState === SyncState.Syncing && state === SyncState.Syncing) {
     ...
     */
    private onTimelineReset = async (room: Room): Promise<void> => {
        if (room === null) return;
    ...
    private eventToJson(ev: MatrixEvent): IEvent {
    ...
    private async crawlerFunc(): Promise<void> {
        ...
        while (!cancelled) {
            ...
            const events = filteredEvents.map((ev) => {
                const event = this.eventToJson(ev);
                const profile = profiles.get(event.sender) ?? {};
                return { event, profile } as IEventAndProfile;
            });

            let newCheckpoint: ICrawlerCheckpoint;
     ...
     *
     * @param {EventTimelineSet} timelineSet The TimelineSet the Timeline belongs to,
