...
export default class CreateSecretStorageDialog extends React.PureComponent {
    ...
    _getInitialPhase() {
        const keyFromCustomisations =
            SecurityCustomisations.createSecretStorageKey?.();
        if (keyFromCustomisations) {
            console.log(
                "Created key via customisations, jumping to bootstrap step",
            );
            this._recoveryKey = {
    ...
    async _fetchBackupInfo() {
        try {
            ...
            const phase =
                backupInfo && !forceReset
                    ? PHASE_MIGRATE
                    : PHASE_CHOOSE_KEY_PASSPHRASE;
    ...
    async _queryKeyUploadAuth() {
        try {
            ...
            const canUploadKeysWithPasswordOnly = error.data.flows.some((f) => {
                return (
                    f.stages.length === 1 && f.stages[0] === "m.login.password"
                );
            });
            this.setState({
        ...
        if (
            ...
            const dialogAesthetics = {
                ...
                [SSOAuthEntry.PHASE_POSTAUTH]: {
                    ...
                    body: _t(
                        "Click the button below to confirm setting up encryption.",
                    ),
    ...
    _renderOptionKey() {
        return (
            <StyledRadioButton
                ...
                name="keyPassphrase"
                checked={
                    this.state.passPhraseKeySelected ===
                    CREATE_STORAGE_OPTION_KEY
                }
    ...
    _renderOptionPassphrase() {
        return (
            <StyledRadioButton
                ...
                checked={
                    this.state.passPhraseKeySelected ===
                    CREATE_STORAGE_OPTION_PASSPHRASE
                }
                onChange={this._onKeyPassphraseChange}
                outlined
            >
    ...
    _renderPhaseChooseKeyPassphrase() {
        const setupMethods = getSecureBackupSetupMethods();
        const optionKey = setupMethods.includes("key")
            ? this._renderOptionKey()
            : null;
        const optionPassphrase = setupMethods.includes("passphrase")
            ? this._renderOptionPassphrase()
            : null;

        return (
            <form onSubmit={this._onChooseKeyPassphraseFormSubmit}>
    ...
    _renderPhaseMigrate() {
        ...
        if (this.state.canUploadKeysWithPasswordOnly) {
                ...
                <div>
                    ...
                    <div>
                        <Field
                            ...
                            forceValidity={
                                this.state.accountPasswordCorrect === false
                                    ? false
                                    : null
                            }
                            autoFocus={true}
            ...
            authPrompt = (
                <p>
                    {_t(
                        "You'll need to authenticate with the server to confirm the upgrade.",
                    )}
                </p>
            );
        }
        ...
        return (
            <form onSubmit={this._onMigrateFormSubmit}>
                <p>
                    {_t(
    ...
    _renderPhasePassPhrase() {
        return (
            <form onSubmit={this._onPassPhraseNextClick}>
                <p>
                    {_t(
                ...
                <div className="mx_CreateSecretStorageDialog_passPhraseContainer">
                    <PassphraseField
                        ...
                        labelStrongPassword={_td(
                            "Great! This Security Phrase looks strong enough.",
                        )}
                        labelAllowedButUnsafe={_td(
                            "Great! This Security Phrase looks strong enough.",
                        )}
                    />
                </div>
    ...
    _renderPhasePassPhraseConfirm() {
        ...
        return (
            <form onSubmit={this._onPassPhraseConfirmNextClick}>
                <p>
                    {_t(
        ...
        return (
            <BaseDialog
                ...
                hasCancel={
                    ...
                    [PHASE_PASSPHRASE].includes(this.state.phase)
                }
                fixedWidth={false}
            >
                <div>{content}</div>
