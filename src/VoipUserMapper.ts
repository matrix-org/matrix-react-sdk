...
export default class VoipUserMapper {
    ...
    public static sharedInstance(): VoipUserMapper {
        if (window.mxVoipUserMapper === undefined)
            window.mxVoipUserMapper = new VoipUserMapper();
        return window.mxVoipUserMapper;
    }

    private async userToVirtualUser(userId: string): Promise<string> {
        const results = await CallHandler.sharedInstance().sipVirtualLookup(
            userId,
        );
        if (results.length === 0 || !results[0].fields.lookup_success)
            return null;
        return results[0].userid;
    }

    public async getOrCreateVirtualRoomForRoom(
        roomId: string,
    ): Promise<string> {
        ...
        const virtualRoomId = await ensureVirtualRoomExists(
            MatrixClientPeg.get(),
            virtualUser,
            roomId,
        );
    ...
    public nativeRoomForVirtualRoom(roomId: string): string {
        ...
        const virtualRoomEvent = virtualRoom.getAccountData(
            VIRTUAL_ROOM_EVENT_TYPE,
        );
    ...
    public isVirtualRoom(room: Room): boolean {
        ...
        const roomCreateEvent = room.currentState.getStateEvents(
            "m.room.create",
            "",
        );
        ...
        // to be invisible)
        if (roomCreateEvent.getSender() !== MatrixClientPeg.get().getUserId())
            return false;
        const claimedNativeRoomId =
            roomCreateEvent.getContent()[VIRTUAL_ROOM_EVENT_TYPE];
    ...
    public async onNewInvitedRoom(invitedRoom: Room): Promise<void> {
        ...
        console.log(
            `Checking virtual-ness of room ID ${invitedRoom.roomId}, invited by ${inviterId}`,
        );
        const result = await CallHandler.sharedInstance().sipNativeLookup(
            inviterId,
        );
        if (result.length === 0) {
            return;
        ...
        if (result[0].fields.is_virtual) {
            ...
            this.virtualToNativeRoomIdCache.set(
                invitedRoom.roomId,
                nativeRoom.roomId,
            );
        }
