...
export default class DevicesPanel extends React.Component {
    constructor(props) {
        ...
        this._onDeviceSelectionToggled =
            this._onDeviceSelectionToggled.bind(this);
            ...
            .then(
                ...
                (error) => {
                    ...
                    if (error.httpStatus == 404) {
                        ...
                        errtxt = _t(
                            "Your homeserver does not support session management.",
                        );
                    } else {
                ...
                Modal.createTrackedDialog(
                    ...
                    InteractiveAuthDialog,
                    {
                        ...
                        aestheticsForStagePhases: {
                            ...
                            [SSOAuthEntry.UNSTABLE_LOGIN_TYPE]:
                                dialogAesthetics,
                        },
                    },
                );
    ...
    _makeDeleteRequest(auth) {
        return MatrixClientPeg.get()
            .deleteMultipleDevices(this.state.selectedDevices, auth)
            .then(() => {
                // Remove the deleted devices from `devices`, reset selection to []
                this.setState({
                    devices: this.state.devices.filter(
                        (d) =>
                            !this.state.selectedDevices.includes(d.device_id),
                    ),
                    selectedDevices: [],
                });
            });
    ...
    _renderDevice(device) {
        const DevicesPanelEntry = sdk.getComponent(
            "settings.DevicesPanelEntry",
        );
        return (
            <DevicesPanelEntry
    ...
    render() {
        ...
        if (this.state.deviceLoadError !== undefined) {
            ...
            return <div className={classes}>{this.state.deviceLoadError}</div>;
        }
