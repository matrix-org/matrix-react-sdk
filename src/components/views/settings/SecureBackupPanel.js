...
export default class SecureBackupPanel extends React.PureComponent {
    ...
    async _checkKeyBackupStatus() {
        ...
        try {
            const { backupInfo, trustInfo } =
                await MatrixClientPeg.get().checkKeyBackup();
            this.setState({
                loading: false,
                error: null,
                backupInfo,
    ...
    async _getUpdatedDiagnostics() {
        ...
        const backupKeyFromCache =
            await cli.crypto.getSessionBackupPrivateKey();
    ...
    render() {
        ...
        if (error) {
            ...
            if (!MatrixClientPeg.get().getKeyBackupEnabled()) {
                ...
                uploadStatus = (
                    <div>
                        {_t("Backing up %(sessionsRemaining)s keys...", {
                            sessionsRemaining,
                        })}{" "}
                        <br />
                    </div>
            ...
            let backupSigStatuses = backupSigStatus.sigs.map((sig, i) => {
                const deviceName = sig.device
                    ? sig.device.getDisplayName() || sig.device.deviceId
                    : null;
                const validity = (sub) => (
                    <span
                        className={
                ...
                const device = (sub) => (
                    <span className="mx_SecureBackupPanel_deviceName">
                        {deviceName}
                    </span>
                );
                const fromThisDevice =
                    sig.device &&
                    sig.device.getFingerprint() ===
                        MatrixClientPeg.get().getDeviceEd25519Key();
                const fromThisUser =
                    sig.crossSigningId &&
        ...
        if (actions.length) {
            actionRow = (
                <div className="mx_SecureBackupPanel_buttonRow">{actions}</div>
            );
        }
