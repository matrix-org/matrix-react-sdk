...
export default class EditHistoryMessage extends React.PureComponent {
    ...
    constructor(props) {
        ...
        if (event.localRedactionEvent()) {
            event
                .localRedactionEvent()
                .on("status", this._onAssociatedStatusChanged);
        }
    ...
    _onRedactClick = async () => {
        const event = this.props.mxEvent;
        const cli = MatrixClientPeg.get();
        const ConfirmAndWaitRedactDialog = sdk.getComponent(
            "dialogs.ConfirmAndWaitRedactDialog",
        );
    ...
    pillifyLinks() {
        ...
        if (this._content.current) {
            pillifyLinks(
                this._content.current.children,
                this.props.mxEvent,
                this._pills,
            );
        }
    ...
    componentWillUnmount() {
        ...
        if (event.localRedactionEvent()) {
            event
                .localRedactionEvent()
                .off("status", this._onAssociatedStatusChanged);
        }
    }
    ...
    render() {
        ...
        if (mxEvent.isRedacted()) {
            ...
            if (this.props.previousEdit) {
                contentElements = editBodyDiffToHtml(
                    getReplacedContent(this.props.previousEdit),
                    content,
                );
            } else {
        ...
        const timestamp = formatTime(
            new Date(mxEvent.getTs()),
            this.props.isTwelveHour,
        );
        const isSending =
            ["sending", "queued", "encrypting"].indexOf(
