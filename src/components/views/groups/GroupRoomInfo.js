...
export default class GroupRoomInfo extends React.Component {
    ...
    _onRemove = (e) => {
        ...
        Modal.createTrackedDialog(
            "Confirm removal of group from room",
            "",
            QuestionDialog,
            {
                title: _t(
                    "Are you sure you want to remove '%(roomName)s' from %(groupId)s?",
                    { roomName, groupId },
                ),
                description: _t(
                    "Removing a room from the community will also remove it from the community page.",
                ),
                button: _t("Remove"),
                onFinished: (proceed) => {
                    if (!proceed) return;
                    ...
                    GroupStore.removeRoomFromGroup(this.props.groupId, roomId)
                        .then(() => {
                            dis.dispatch({
                        ...
                        .catch((err) => {
                            console.error(
                                `Error whilst removing ${roomId} from ${groupId}`,
                                err,
                            );
                            ...
                            Modal.createTrackedDialog(
                                ...
                                {
                                    title: _t(
                                        "Failed to remove room from community",
                                    ),
                                    description: _t(
    ...
    _changeGroupRoomPublicity = (e) => {
        ...
        const roomId = this.props.groupRoomId;
        const roomName = this.state.groupRoom.displayname;
        GroupStore.updateGroupRoomVisibility(
            this.props.groupId,
            roomId,
            isPublic,
        )
            .catch((err) => {
                console.error(
                    `Error whilst changing visibility of ${roomId} in ${groupId} to ${isPublic}`,
                    err,
                );
