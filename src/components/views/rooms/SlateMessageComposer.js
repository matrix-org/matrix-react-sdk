...
export default class SlateMessageComposer extends React.Component {
    constructor(props, context) {
        ...
        this.onToggleFormattingClicked = this.onToggleFormattingClicked.bind(
            this
        );
        ...
        this.state = {
            inputState: {
                marks: [],
                blockType: null,
                isRichTextEnabled: SettingsStore.getValue(
                    "MessageComposerInput.isRichTextEnabled"
                )
            },
            showFormatting: SettingsStore.getValue(
    ...
    onToggleMarkdownClicked(e) {
        ...
        this.messageComposerInput.enableRichtext(
            !this.state.inputState.isRichTextEnabled
        );
    }
    ...
    renderPlaceholderText() {
        const roomIsEncrypted = MatrixClientPeg.get().isRoomEncrypted(
    ...
    renderFormatBar() {
        ...
        const formatButtons = formatButtonList.map(name => {
            ...
            const active =
                marks.some(mark => mark.type === markName) ||
                blockType === name;
            const suffix = active ? "-on" : "";
            const onFormatButtonClicked = this.onFormatButtonClicked.bind(
                this,
                name
            );
            const className =
                "mx_MessageComposer_format_button mx_filterFlipColor";
            return (
                <img
                    className={className}
                    title={_t(name)}
        ...
        return (
            <div className="mx_MessageComposer_formatbar_wrapper">
                <div className="mx_MessageComposer_formatbar">
                    ...
                    <AccessibleButton
                        element="img"
                        title={_t("Hide Text Formatting Toolbar")}
    ...
    render() {
        ...
        if (!this.state.tombstone && this.state.canSendMessages) {
            ...
            const MessageComposerInput = sdk.getComponent(
                "rooms.MessageComposerInput"
            );
            const showFormattingButton = this.state.inputState
                .isRichTextEnabled;
