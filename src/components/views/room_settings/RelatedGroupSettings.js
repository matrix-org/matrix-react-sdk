...
export default class RelatedGroupSettings extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = {
            newGroupId: "",
            newGroupsList: props.relatedGroupsEvent
                ? props.relatedGroupsEvent.getContent().groups || []
                : [],
        };
    }
    ...
    validateGroupId(groupId) {
        if (!GROUP_ID_REGEX.test(groupId)) {
            ...
            Modal.createTrackedDialog(
                "Invalid related community ID",
                "",
                ErrorDialog,
                {
                    title: _t("Invalid community ID"),
                    description: _t(
                        "'%(groupId)s' is not a valid community ID",
                        { groupId },
                    ),
                },
            );
    ...
    onGroupDeleted = (index) => {
        ...
        const newGroupsList = this.state.newGroupsList.filter(
            (g) => g !== group,
        );
        ...
        return (
            <div>
                <EditableItemList
                    ...
                    placeholder={_t(
                        "New community ID (e.g. +foo:%(localDomain)s)",
                        { localDomain },
                    )}
                />
            </div>
