...
export default class PersistentApp extends React.Component {
    ...
    componentWillUnmount() {
        ...
        ActiveWidgetStore.removeListener(
            "update",
            this._onActiveWidgetStoreUpdate,
        );
        if (MatrixClientPeg.get()) {
            MatrixClientPeg.get().removeListener(
                "Room.myMembership",
                this._onMyMembership,
            );
        }
    }
    ...
    _onMyMembership = async (room, membership) => {
        const persistentWidgetInRoomId = ActiveWidgetStore.getRoomId(
            this.state.persistentWidgetId,
        );
        if (membership !== "join") {
    ...
    render() {
        if (this.state.persistentWidgetId) {
            ...
            const persistentWidgetInRoom = MatrixClientPeg.get().getRoom(
            ...
            if (
                this.state.roomId !== persistentWidgetInRoomId &&
                myMembership === "join"
            ) {
                // get the widget data
                const appEvent = WidgetUtils.getRoomWidgets(
                    persistentWidgetInRoom,
                ).find((ev) => {
                    return (
                        ev.getStateKey() ===
                        ActiveWidgetStore.getPersistentWidgetId()
                    );
                });
                const app = WidgetUtils.makeAppConfig(
                    appEvent.getStateKey(),
                    appEvent.getContent(),
                    appEvent.getSender(),
                    persistentWidgetInRoomId,
                    appEvent.getId(),
