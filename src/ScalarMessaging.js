...
function inviteUser(event, roomId, userId) {
    ...
    if (!client) {
        sendError(event, counterpart.translate("You need to be logged in") + ".");
        return;
    }
    ...
    client.invite(roomId, userId).done(function() {
        ...
        sendError(event, counterpart.translate("You need to be able to invite users to do that") + ".", err);
    });
}
...
function setPlumbingState(event, roomId, status) {
    ...
    client.sendStateEvent(roomId, "m.room.plumbing", { status : status }).done(() => {
        ...
        sendError(event, err.message ? err.message : counterpart.translate("Failed to send request") + ".", err);
    });
}
...
function setBotPower(event, roomId, userId, level) {
    if (!(Number.isInteger(level) && level >= 0)) {
        sendError(event, counterpart.translate("Power level must be positive integer") + ".");
        return;
    }
...
function returnStateEvent(event, roomId, eventType, stateKey) {
    ...
    const room = client.getRoom(roomId);
    if (!room) {
        sendError(event, counterpart.translate("This room is not recognised") + ".");
        return;
    }
...
const onMessage = function(event) {
    ...
    if (!roomId) {
        sendError(event, counterpart.translate("Missing room_id in request"));
        return;
    }
    let promise = Promise.resolve(currentRoomId);
    if (!currentRoomId) {
        if (!currentRoomAlias) {
            sendError(event, counterpart.translate("Must be viewing a room"));
            return;
        }
    ...
    promise.then((viewingRoomId) => {
        if (roomId !== viewingRoomId) {
            sendError(event, counterpart.translate("Room %(roomId)s not visible", {roomId: roomId}));
            return;
        }
        ...
        if (!userId) {
            sendError(event, counterpart.translate("Missing user_id in request"));
            return;
        }
        ...
        console.error(err);
        sendError(event, counterpart.translate("Failed to lookup current room") + ".");
    });
};
