...
export default class ScalarAuthClient {
    ...
    constructor(private apiUrl: string, private uiUrl: string) {
        ...
        this.isDefaultManager =
            apiUrl === configApiUrl && configUiUrl === uiUrl;
    }

    private writeTokenToStore() {
        window.localStorage.setItem(
            "mx_scalar_token_at_" + this.apiUrl,
            this.scalarToken,
        );
    ...
    private checkToken(token: string): Promise<string> {
        return this.getAccountName(token)
            .then((userId) => {
                ...
                if (userId !== me) {
                    throw new Error(
                        "Scalar token is owned by someone else: " + me,
                    );
                }
                return token;
            })
            .catch((e) => {
                if (e instanceof TermsNotSignedError) {
                    console.log(
                        "Integration manager requires new terms to be agreed to",
                    );
            ...
            request(
                ...
                (err, response, body) => {
                    if (err) {
                        ...
                        reject(
                            new Error(
                                `Scalar request failed: ${response.statusCode}`,
                            ),
                        );
            ...
            request(
                ...
                (err, response, body) => {
                    if (err) {
                        ...
                        let title = "";
                        if (
                            body.page_title_cache_item &&
                            body.page_title_cache_item.cached_title
                        ) {
    ...
    getStarterLink(starterLinkUrl: string): string {
        return (
            starterLinkUrl +
            "?scalar_token=" +
            encodeURIComponent(this.scalarToken)
        );
    }
}
