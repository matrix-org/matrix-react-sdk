    ...
    if (client) {
        ...
        if (client.isCryptoEnabled()) {
            ...
            body.append(
                ...
                String(
                    !!(await crossSigning.isStoredInSecretStorage(
                        secretStorage,
                    )),
                ),
            );
 ...
 */
export default async function sendBugReport(
...
export async function downloadBugReport(opts: IOpts = {}) {
    ...
    dl.href = `data:application/octet-stream;base64,${btoa(
        uint8ToString(tape.out),
    )}`;
