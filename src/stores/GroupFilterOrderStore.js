...
class GroupFilterOrderStore extends Store {
    ...
    __onDispatch(payload) {
        ...
        switch (payload.action) {
            ...
            case "setting_updated":
                if (
                    payload.settingName === "TagPanel.enableTagPanel" &&
                    !payload.newValue
                ) {
                    this._setState({
                        selectedTags: [],
    ...
    _mergeGroupsAndTags() {
        ...
        const tagsToKeep = tags.filter(
            (t) =>
                (t[0] !== "+" || groupIds.includes(t)) && !removedTags.has(t),
        );

        const cli = MatrixClientPeg.get();
        const migratedCommunities = new Set(
            cli
                .getRooms()
                .map((r) => {
                    return r.currentState
                        .getStateEvents(EventType.RoomCreate, "")
                        ?.getContent()[CreateEventField];
                })
                .filter(Boolean),
        );
        const groupIdsToAdd = groupIds.filter(
            (groupId) =>
                !tags.includes(groupId) &&
                !removedTags.has(groupId) &&
                !migratedCommunities.has(groupId),
        );
