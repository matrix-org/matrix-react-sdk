...
type HandlerMap = {
    releaseAnnouncementChanged: (newName: Features) => void;
};
...
export class ReleaseAnnouncementStore extends TypedEventEmitter<ReleaseAnnouncementStoreEvents, HandlerMap> {
    ...
    private currentReleaseAnnouncement = 0;
    ...
    public getReleaseAnnouncement(): Features {
        if (this.currentReleaseAnnouncement >= FEATURES.length) {
            this.currentReleaseAnnouncement = 0;
        }

        return FEATURES[this.currentReleaseAnnouncement];
    }

    public nextReleaseAnnouncement(): void {
        this.currentReleaseAnnouncement++;
