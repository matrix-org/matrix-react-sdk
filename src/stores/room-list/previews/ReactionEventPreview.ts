...
export class ReactionEventPreview implements IPreview {
    public getTextFor(event: MatrixEvent, tagId?: TagID): string {
        const showDms = SettingsStore.getValue(
            "feature_roomlist_preview_reactions_dms",
        );
        const showAll = SettingsStore.getValue(
        ...
        if (!showAll) {
            ...
            if (
                !(
                    showDms &&
                    DMRoomMap.shared().getUserIdForRoomId(event.getRoomId())
                )
            ) {
                return null;
            }
        ...
        if (
            ...
            return _t("%(senderName)s: %(reaction)s", {
