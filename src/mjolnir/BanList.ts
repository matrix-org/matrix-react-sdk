...
export function ruleTypeToStable(rule: string, unstable = true): string {
    if (USER_RULE_TYPES.includes(rule)) {
        return unstable
            ? USER_RULE_TYPES[USER_RULE_TYPES.length - 1]
            : RULE_USER;
    }
    if (ROOM_RULE_TYPES.includes(rule)) {
        return unstable
            ? ROOM_RULE_TYPES[ROOM_RULE_TYPES.length - 1]
            : RULE_ROOM;
    }
    if (SERVER_RULE_TYPES.includes(rule)) {
        return unstable
            ? SERVER_RULE_TYPES[SERVER_RULE_TYPES.length - 1]
            : RULE_SERVER;
    }
    return null;
}
...
export class BanList {
    ...
    async banEntity(
        ...
        await MatrixClientPeg.get().sendStateEvent(
            this._roomId,
            ruleTypeToStable(kind, true),
            {
                entity: entity,
                reason: reason,
                recommendation: recommendationToStable(
                    RECOMMENDATION_BAN,
                    true,
                ),
            },
    ...
    updateList() {
        ...
        for (const eventType of ALL_RULE_TYPES) {
            ...
            for (const ev of events) {
                ...
                this._rules.push(
                    new ListRule(entity, recommendation, reason, kind),
                );
            }
        }
    }
