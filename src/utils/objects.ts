...
type ObjectExcluding<O extends {}, P extends (keyof O)[]> = {
 ...
 */
export function objectExcluding<O extends {}, P extends Array<keyof O>>(
    a: O,
    props: P,
): ObjectExcluding<O, P> {
    // We use a Map to avoid hammering the `delete` keyword, which is slow and painful.
    const tempMap = new Map<keyof O, any>(
        Object.entries(a) as [keyof O, any][],
    );
 ...
 */
export function objectWithOnly<O extends {}, P extends Array<keyof O>>(
    a: O,
    props: P,
): { [k in P[number]]: O[k] } {
 ...
 */
export function objectShallowClone<O extends {}>(
    a: O,
    propertyCloner?: (k: keyof O, v: O[keyof O]) => any,
): O {
    const newObj = {} as O;
