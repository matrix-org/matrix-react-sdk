...
export default class WidgetUtils {
    ...
    static isScalarUrl(testUrlString: string): boolean {
        ...
        if (!scalarUrls || scalarUrls.length === 0) {
            const defaultManager =
                IntegrationManagers.sharedInstance().getPrimaryManager();
            if (defaultManager) {
    ...
    static waitForUserWidget(widgetId: string, add: boolean): Promise<void> {
        return new Promise((resolve, reject) => {
            ...
            const startingAccountDataEvent =
                MatrixClientPeg.get().getAccountData("m.widgets");
            ...
            const timerId = setTimeout(() => {
                ...
                reject(
                    new Error(
                        "Timed out waiting for widget ID " +
                            widgetId +
                            " to appear",
                    ),
                );
        ...
        return new Promise((resolve, reject) => {
            ...
            function eventsInIntendedState(evList) {
                const widgetPresent = evList.some((ev) => {
                    return (
                        ev.getContent() && ev.getContent()["id"] === widgetId
                    );
                });
                if (add) {
            ...
            // TODO: Enable support for m.widget event type (https://github.com/vector-im/element-web/issues/13111)
            const startingWidgetEvents = room.currentState.getStateEvents(
                "im.vector.modular.widgets",
            );
            ...
            function onRoomStateEvents(ev) {
                ...
                // TODO: Enable support for m.widget event type (https://github.com/vector-im/element-web/issues/13111)
                const currentWidgetEvents = room.currentState.getStateEvents(
                    "im.vector.modular.widgets",
                );
    ...
    static setRoomWidgetContent(
        ...
        return client
            ...
            .then(() => {
                return WidgetUtils.waitForRoomWidget(
                    widgetId,
                    roomId,
                    addingWidget,
                );
    ...
    static getRoomWidgets(room: Room) {
        // TODO: Enable support for m.widget event type (https://github.com/vector-im/element-web/issues/13111)
        const appsStateEvents = room.currentState.getStateEvents(
            "im.vector.modular.widgets",
        );
        if (!appsStateEvents) {
            return [];
    ...
    static getStickerpickerWidgets(): IWidgetEvent[] {
        ...
        return widgets.filter(
            (widget) =>
                widget.content && widget.content.type === "m.stickerpicker",
        );
    }
    ...
    static getIntegrationManagerWidgets(): IWidgetEvent[] {
        const widgets = WidgetUtils.getUserWidgetsArray();
        return widgets.filter(
            (w) => w.content && w.content.type === "m.integration_manager",
        );
    }
    ...
    static async removeIntegrationManagerWidgets(): Promise<void> {
        ...
        if (!widgets) return;
        const userWidgets: Record<string, IWidgetEvent> =
            widgets.getContent() || {};
        Object.entries(userWidgets).forEach(([key, widget]) => {
            if (
                widget.content &&
                widget.content.type === "m.integration_manager"
            ) {
    ...
    static makeAppConfig(
        ...
        if (!senderUserId) {
            throw new Error(
                "Widgets must be created by someone - provide a senderUserId",
            );
        }
    ...
    static getCapWhitelistForAppTypeInRoomId(
        appType: string,
        roomId: string,
    ): Capability[] {
        const enableScreenshots = SettingsStore.getValue(
            "enableWidgetScreenshots",
            roomId,
        );

        const capWhitelist = enableScreenshots
    ...
    static getLocalJitsiWrapperUrl(
        opts: { forLocalRender?: boolean; auth?: string } = {},
    ) {
