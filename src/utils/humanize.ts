...
export function humanizeTime(timeMillis: number): string {
    ...
    if (msAgo >= 0) {
        ...
        if (minutes <= MINUTES_UNDER_1_HOUR)
            return _t("%(num)s minutes ago", { num: minutes });
        if (minutes <= MINUTES_1_HOUR) return _t("about an hour ago");
        if (hours <= HOURS_UNDER_1_DAY)
            return _t("%(num)s hours ago", { num: hours });
        ...
        if (minutes <= MINUTES_UNDER_1_HOUR)
            return _t("%(num)s minutes from now", { num: minutes });
        if (minutes <= MINUTES_1_HOUR) return _t("about an hour from now");
        if (hours <= HOURS_UNDER_1_DAY)
            return _t("%(num)s hours from now", { num: hours });
