...
const enStringsRaw = JSON.parse(fs.readFileSync(path.join(I18NDIR, 'en_EN.json')).toString());
...
for (const filename of fs.readdirSync(I18NDIR)) {
    if (filename === 'en_EN.json')
        continue;
    if (filename === 'basefile.json')
        continue;
    if (!filename.endsWith('.json'))
        continue;
    const trs = JSON.parse(fs.readFileSync(path.join(I18NDIR, filename)).toString());
