import { _t, _tDom, TranslatedString, setLanguage, setMissingEntryGenerator, substitute } from '../../src/languageHandler';
...
describe('languageHandler', function () {
    ...
    const testCasesEn: TestCase[] = [
        ...
        ['handles simple tag substitution', selfClosingTagSub, {}, { 'policyLink': () => 'foo' }, 'Accept foo to continue:'],
        ['handles text in tags', textInTagSub, {}, { 'a': (sub) => `x${sub}x` }, 'xUpgradex to your own domain'],
        ['handles variable substitution with React function component', variableSub, { userId: () => <i>foo</i> }, undefined, <span>You are now ignoring <i>foo</i></span>],
        ['handles variable substitution with react node', variableSub, { userId: <i>foo</i> }, undefined, <span>You are now ignoring <i>foo</i></span>],
        ['handles tag substitution with React function component', selfClosingTagSub, {}, { 'policyLink': () => <i>foo</i> }, <span>Accept <i>foo</i> to continue:</span>],
    ];
    ...
    describe('when a translation string does not exist in active language', () => {
        ...
        describe('_t', () => {
            it.each(testCasesEn)("%s and translates with fallback locale", async (_d, translationString, variables, tags, result) => {
                expect(_t(translationString, variables, tags)).toEqual(result);
            });
        })

        describe('_tDom()', () => {
            it.each(testCasesEn)("%s and translates with fallback locale, attributes fallback locale", async (_d, translationString, variables, tags, result) => {
