...
import MemberListViewModel, { IMemberListViewModel } from "../../src/view-models/rooms/memberlist/MemberListViewModel";
...
describe("MemberListViewModel", () => {
    ...
    async function loadViewModel(): Promise<RenderResult<IMemberListViewModel>> {
        let vm = renderHook(() => MemberListViewModel(true, true, memberListService));
        const { result } = vm;
    ...
    it("initial loaded states", async () => {
        ...
        // loading state toggles correctly when loading the member list
        expect(result.all[0]).toEqual(expect.objectContaining({loading: false }))
        expect(result.all[1]).toEqual(expect.objectContaining({loading: true }))
        expect(result.all[2]).toEqual(expect.objectContaining({loading: false }))
